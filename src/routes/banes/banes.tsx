...
export default function Boons() {
  ...
  const boonList = Object.values(banes.banes).filter((boon) => boon?.name?.toLocaleLowerCase()?.includes(searchText?.toLocaleLowerCase()));
  return (
    <Container>
      ...
      <div>
        {boonList.map((boon) => (
          <Card
            className="no-break"
            key={boon.name}
            onClick={() => navigate(`/bane/${boon.name}`)}
            sx={{ mb: 1 }}
          >
            <CardActionArea
              sx={{ p: 1 }}
            >
              ...
              <Typography>
                {boon.attackAttributes.map((attribute) => (
